...
int main(int argc, char **argv)
{
    ...
    std::string pattern = "/home/pxw/project/traffic_light/data/test/";
    ...
    tl_1->status.signal = perception::base::TLSignal::TL_STRAIGHT;
    ...
    tl_2->status.signal = perception::base::TLSignal::TL_STRAIGHT;
    ...
    for (auto it : file_names)
    {
        ...
        for (auto it : frame.traffic_lights)
        {
            ...
            std::cout << "Signal: " << static_cast<int>(it->status.signal) << std::endl;
            ...
            // 写上颜色和转向信息
            std::stringstream ss;
            ss << it->status.track_id << "_" << static_cast<int>(it->status.color) << "_" << static_cast<int>(it->status.signal);
